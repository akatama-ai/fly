$( document ).ready(function() {
  ...
  function update_withdrawal_authenticator(status) {
      delay(function() {
          $.ajax({
              url: $('#link_update').data('link'),
              type: "post",
              dateType: "text",
              data: {
                  data: $('#update_authenticator_withdrawal input[type=radio]:checked').val()
              },
              success: function(result) {
                  result = $.parseJSON(result);
                if (result.status == 1){
  ...
  $('#update_authenticator_withdrawal input[type=radio]').change(function() {
         alertify.confirm('<p class="text-center" style="font-size:25px;color: black;text-transform: ;height: 20px">Do you want update withdrawal use Authenticator ?</p>', function(e) {
            if (e) {
                update_withdrawal_authenticator(1);
...
function readURL(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                $('#blah').attr('src', e.target.result).show().css({'width': '150px','height':'150px'});
...
function readURLs(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                $('#blahs').attr('src', e.target.result).show().css({'width': '250px','height':'250px','display':'block'});
